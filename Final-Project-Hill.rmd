...
```{r}
checkboxGroupInput("Weather", label = h3("Weather Condition"), 
    choices = list("Blowing Sand Soil Dirt", "Clear", "Cloudy", "Fog Smog Smoke", "Other", "Rain", "Severe Crosswinds", "Unknown", "Sleet Hail Freezing Rain Or Drizzle"),
    selected = c("Cloudy", "Rain"))
...
### Traffic Accidents Based on Weather Conditions And Time of Day

```{r}
renderLeaflet({
  
  we.con <- input$Weather  
...
#  d3$col.vec3 <- ifelse( d2$nohurt, "gray20", ifelse(d2$inj, "steelblue", "darkorange") )              
    
 point.size <- d3$Totalinjuries + d3$Totalfatalities

  crash.details <- paste0( "Time: ", d3$DateTime, "<br>",
                    
                     "Weather Condition: ", d3$Weather)
  ...
  addCircles( tempe, lng=d3$Longitude, lat=d3$Latitude
              , fillOpacity=0.5, 
...
Traffic Accidents By Collision Type
=====================================  
Inputs {.sidebar}
-------------------------------------

```{r}
checkboxGroupInput("c.m", label = h3("Collision Type"), 
    choices = list("ANGLE (Front To Side)(Other Than Left Turn)", "Head On", "Left Turn", "Other", "Rear End", "Rear To Rear", "Rear To Side", "Sideswipe Opposite Direction", "Sideswipe Same Direction", "Single Vehicle", "Unknown"),
    selected = c("Unknown"))
...
renderLeaflet({
  ...
  d0 <-
    dat %>%
    filter( Collisionmanner %in% input$c.m)
  
  d0$col.m <- ifelse( d0$nohurt, "gray20", ifelse(d0$inj, "steelblue", "darkorange") )              

  crash.details1 <- paste0( "Time: ", d0$DateTime, "<br>",
                    
                     "Collision type: ", d0$Collisionmanner)
  ...
  addCircles( tempe, lng=d0$Longitude, lat=d0$Latitude,
               fillOpacity=1, 
              stroke=F,
            )
...
This dashboard is intended to educate users on accidents in Tempe, AZ. The data was provided by the Tempe Open Data Portal and is available at: https://data.tempe.gov/dataset/high-severity-traffic-crashes-1-08
